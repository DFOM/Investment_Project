...
class DatabaseCompatibilityTests(unittest.TestCase):
    ...
    def test_trade_executor_enforces_decimal_types_before_balance_math(self) -> None:
        body = Path("core/trade_executor.py").read_text(encoding="utf-8")

        self.assertIn("gross_jpy_mid = _d(usd_notional_decimal * live_mid_fx_rate)", body)
        self.assertIn("commission_paid = _d(calculate_commission(symbol, gross_jpy_mid, live_mid_fx_rate))", body)
        self.assertIn("fx_conversion_fee_paid = _d(fx_conversion_fee_paid)", body)
        self.assertIn("remaining_balance = _d(previous_balance) + _d(total_jpy_impact)", body)
