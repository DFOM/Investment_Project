...
def _current_holdings(trader_name: str | None = None) -> dict[str, Decimal]:
    ...
    for _, row in frame.iterrows():
        ...
        if not ticker:
            continue
        quantity = _d(row.get("Quantity", 0) or 0)
        action = str(row.get("Action", "")).strip().upper()
        if action == "BUY":
            totals[ticker] = totals.get(ticker, Decimal("0")) + quantity
        elif action == "SELL":
            totals[ticker] = totals.get(ticker, Decimal("0")) - quantity
...
def execute_trade(
    ...
    gross_jpy_mid = _d(usd_notional_decimal * live_mid_fx_rate)
    
    # Calculate commission using Rakuten rates
    commission_paid = _d(calculate_commission(symbol, gross_jpy_mid, live_mid_fx_rate))
    fx_conversion_fee_paid = _d(fx_conversion_fee_paid)
    ...
    remaining_balance = _d(previous_balance) + _d(total_jpy_impact)
