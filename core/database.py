...
def _resolved_cash_balances_by_member(ledger: pd.DataFrame) -> dict[str, float]:
    """Return latest cash per active member, using allocations for members with no trades."""
    allocations = resolve_member_initial_allocations()
    raw_balances = _latest_cash_balances_by_trader(ledger)
    ignored = {"", "system", "all team"}
    balances: dict[str, float] = {}

    for member, allocation in allocations.items():
        match = next(
            (balance for trader, balance in raw_balances.items() if trader.strip().casefold() == member.casefold()),
            None,
        )
        balances[member] = float(match if match is not None else allocation)

    if balances:
        return balances

    return {
        trader: float(balance)
        for trader, balance in raw_balances.items()
        if trader.strip().casefold() not in ignored
    }
...
def _latest_cash_balance(ledger: pd.DataFrame) -> float:
    ...
    balances = _resolved_cash_balances_by_member(ledger)
    if balances:
...
def _pending_buy_value_jpy(usd_jpy: float, trader_name: str | None = None) -> tuple[float, list[str]]:
    """Estimate pending BUY notional for valuation snapshots without moving cash."""
    from core.market_data import get_live_price
    ...
    frame = orders.copy()
    for column in ["Action", "Ticker", "Trader_Name"]:
        if column not in frame.columns:
            frame[column] = ""
        frame[column] = frame[column].astype(str).str.strip()
    frame["Action"] = frame["Action"].str.upper()
    frame["Ticker"] = frame["Ticker"].str.upper()
    frame["Value"] = pd.to_numeric(frame.get("Value", 0), errors="coerce").fillna(0.0)

    if trader_name:
        frame = frame[frame["Trader_Name"].str.casefold() == trader_name.strip().casefold()]

    frame = frame[frame["Action"].eq("BUY")]
    if frame.empty:
        return 0.0, []

    total = 0.0
    skipped: list[str] = []
    for _, order in frame.iterrows():
        ...
        if price is None:
            skipped.append(ticker)
            continue
        fx = 1.0 if ticker.endswith(".T") else usd_jpy
        total += quantity * float(price) * fx
...
def _record_portfolio_snapshot() -> dict[str, Any]:
    ...
    pending_buy_value, pending_skipped = _pending_buy_value_jpy(usd_jpy)
    outstanding_by_trader = get_outstanding_borrowing_by_member()
    outstanding_total = float(sum(outstanding_by_trader.values())) if outstanding_by_trader else 0.0
    # Pending BUY orders are shown separately/reserved, but not added to total
    # portfolio value because cash has not left the account yet.
    total_jpy = cash + equity_jpy - outstanding_total
    skipped = [*skipped, *pending_skipped]
    today = datetime.now(timezone.utc).date()

    _upsert_daily_performance_row("All Team", Decimal(str(total_jpy)), today)

    cash_by_trader = _resolved_cash_balances_by_member(ledger)
    if cash_by_trader:
        for trader in cash_by_trader:
            if not ledger.empty and "Trader_Name" in ledger.columns:
                trader_df = ledger[
                    ledger["Trader_Name"].astype(str).str.casefold() == str(trader).casefold()
                ].copy()
            else:
            ...
            trader_pending_value, trader_pending_skipped = _pending_buy_value_jpy(usd_jpy, str(trader))
            skipped.extend(trader_pending_skipped)
            # Do not add pending order notional to member total; it is already
            # backed by the member's cash until the market opens and executes.
            trader_borrowing = outstanding_by_trader.get(str(trader), 0.0)
            trader_value = trader_cash + trader_equity - float(trader_borrowing)
    ...
    return {
        ...
        "pending_buy_value_jpy": pending_buy_value,
        "outstanding_borrowing_jpy": outstanding_total,
...
def get_outstanding_borrowing_by_member() -> dict[str, float]:
    """Return outstanding borrowing principal by trader_name."""
    try:
        db = get_postgres_db()
        rows = db.execute_query(
            """
            SELECT trader_name, COALESCE(SUM(amount_jpy - COALESCE(repaid_amount, 0)), 0) AS outstanding
            FROM borrowing
            WHERE status = 'ACTIVE'
        ...
        out: dict[str, float] = {}
        for row in rows or []:
            trader = str(row[0]).strip()
            outstanding = max(0.0, float(row[1] or 0.0))
            if trader:
                out[trader] = outstanding
        return out
    except Exception as exc:
        logger.error(f"Failed to load outstanding borrowing map: {exc}")
        return {}
