...
def main() -> None:
    ...
    total = cash + equity_jpy + pending_buy_value
    if is_all:
    ...
    if not is_all:
        ...
        member_portfolio_value = member_metrics["current_value"] + pending_buy_value
        m4.metric("Member Portfolio Value", format_currency(member_portfolio_value, "JPY"))
    ...
    if pending_order_count:
        st.caption(
            f"Pending BUY orders add an estimated {format_currency(pending_buy_value, 'JPY')} "
            "to portfolio value previews until they execute; cash is not deducted until execution."
        )
    if pending_skipped:
        st.warning(
