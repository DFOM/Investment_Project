...
def main() -> None:
    ...
    try:
        ...
        if not ob_df.empty and "Status" in ob_df.columns:
            ...
            if selected_trader and "Trader_Name" in ob_df.columns:
                aliases = {name.casefold() for name in get_member_aliases(selected_trader)}
                trader_match = ob_df["Trader_Name"].astype(str).str.casefold().isin(aliases)
            else:
                trader_match = pd.Series(True, index=ob_df.index)
            pending = ob_df[normalized_status.eq("PENDING") & trader_match].reset_index(drop=True)
